...
async function submitToAirtable(form) {
  const fields = {
    ...
    creative_role:    form.role           || "",
    years_experience: form.yearsExperience ? Number(form.yearsExperience) : 0,
    portfolio_url:    form.portfolioUrl   || "",
    ai_summary:       form.notes          || "",
    status:           "New",
  };
...
function SectionTitle({ eyebrow, title, desc }) {
  return (
    <div className="max-w-3xl">
      ...
      {desc && <p className="mt-4 text-base leading-8" style={{ color: brand.muted }}>{desc}</p>}
    </div>
  );
}
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {navItems.map((item) => {
            ...
            return (
              <NavLink key={item.path} to={item.path} className="rounded-xl px-4 py-2 text-sm transition"
                style={{ background: active ? brand.primary : "transparent", color: active ? "white" : brand.muted }}>
                {item.label}
              </NavLink>
            );
          })}
        ...
        <div className="flex items-center gap-3">
          <Link to="/contact" className="hidden rounded-2xl px-5 py-3 text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5 md:inline-flex"
            style={{ background: brand.primary }}>
            Book a consultation
          </Link>
          <button className="flex h-11 w-11 items-center justify-center rounded-2xl border bg-white md:hidden"
            style={{ borderColor: brand.line }} onClick={() => setMenuOpen((s) => !s)}>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            className="border-t bg-white px-6 py-4 md:hidden" style={{ borderColor: brand.line }}>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 lg:grid-cols-[1.1fr_0.9fr_0.9fr] lg:px-8">
        ...
        <div>
          ...
          <div className="mt-4 grid gap-2">
            {navItems.map((item) => (
              <NavLink key={item.path} to={item.path} className="w-fit text-sm transition" style={{ color: brand.muted }}>{item.label}</NavLink>
            ))}
          </div>
        </div>
        <div>
          ...
          <motion.div initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.6 }} className="relative z-10 flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link to="/contact" className="inline-flex items-center gap-2 rounded-2xl px-6 py-3.5 text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5" style={{ background: brand.primary }}>
                Start with Masar <ArrowRight className="h-4 w-4" />
              </Link>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[28px] border bg-white p-7 shadow-[0_12px_40px_rgba(15,23,42,0.07)] sm:col-span-2" style={{ borderColor: brand.line }}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="rounded-2xl p-3 text-white" style={{ background: brand.primary }}><BriefcaseBusiness className="h-5 w-5" /></div>
                </div>
              ...
              <div className="rounded-[28px] border p-7 text-white shadow-[0_12px_40px_rgba(15,23,42,0.08)]" style={{ borderColor: brand.primary, background: brand.primary }}>
                ...
                <p className="mt-4 text-sm leading-7 text-slate-300">Ideal for companies that want access to Egyptian talent while keeping visibility, speed, and professional structure.</p>
              </div>
              <div className="rounded-[28px] border bg-white p-7 shadow-[0_12px_40px_rgba(15,23,42,0.07)]" style={{ borderColor: brand.line }}>
                ...
                <p className="mt-4 text-sm leading-7" style={{ color: brand.muted }}>Clearer ownership, smoother processes, and stronger monthly follow-up from first hire onward.</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 lg:px-8">
        <div className="rounded-[34px] border bg-white p-10 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-14" style={{ borderColor: brand.line }}>
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <SectionTitle eyebrow="How it works" title="A simple operational path from brief to monthly execution."
    ...
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <motion.div initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.55 }} className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[32px] bg-white p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-10">
          <SectionTitle eyebrow="About Masar" title="Masar is not only about hiring people. It is about building the system around them."
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-3">
        {pillars.map((item, index) => {
          ...
          return (
            <motion.div key={item.title} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.07 }}
              className="rounded-[30px] border bg-white p-8 shadow-[0_16px_45px_rgba(15,23,42,0.06)]" style={{ borderColor: brand.line }}>
...
function ServicesPage() {
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <SectionTitle eyebrow="Services" title="A premium service structure for building and running teams in Egypt."
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        {services.map((service, index) => {
          ...
          return (
            <motion.div key={service.title} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.06 }}
              className="rounded-[30px] border bg-white p-8 shadow-[0_16px_45px_rgba(15,23,42,0.06)]" style={{ borderColor: brand.line }}>
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl text-white" style={{ background: brand.primary }}><Icon className="h-5 w-5" /></div>
...
function PackagesPage() {
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <SectionTitle eyebrow="Packages" title="Flexible commercial models based on team size and operational depth."
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-3">
        {packages.map((pkg, index) => (
          <motion.div key={pkg.name} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.07 }}
            className="rounded-[32px] border p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)]"
            style={{ borderColor: pkg.featured ? brand.primary : brand.line, background: pkg.featured ? brand.primary : brand.card, color: pkg.featured ? "white" : brand.primary }}>
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="rounded-full px-3 py-1 text-xs font-medium uppercase tracking-[0.18em]"
                style={{ background: pkg.featured ? "rgba(255,255,255,0.08)" : "#F5F2E8", color: pkg.featured ? "white" : brand.secondary }}>
                {pkg.tag}
              </div>
            </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// APPLY PAGE — submits to Google Sheets + Airtable simultaneously
// ─────────────────────────────────────────────────────────────────────────────
function FAQPage() {
  ...
  async function handleSubmit(e) {
    ...
    // ── Step 1: Submit to Google Sheets (original — unchanged) ────────────
    ...
    try {
      ...
      let result;
      try { result = JSON.parse(text); } catch { throw new Error("Server returned an invalid response."); }
    ...
    // ── Step 2: Submit to Airtable (new — triggers Claude AI evaluation) ──
    // Runs in background — if it fails, user experience is unaffected
    submitToAirtable(form).catch((err) => {
      console.warn("Airtable sync (non-critical):", err.message);
    });

    // ── Step 3: Show success to user ──────────────────────────────────────
    setStatus({
      type:    "success",
      message: `Application submitted successfully! Score: ${googleResult.ai?.score ?? "Pending AI review"} | Decision: ${googleResult.ai?.decision ?? "Under review"}`,
    });

    setForm({ fullName: "", email: "", phone: "", country: "", city: "", role: "", yearsExperience: "", portfolioUrl: "", linkedinUrl: "", salaryExpectation: "", notes: "" });
    setLoading(false);
  }

  const inputStyle = { borderColor: brand.line };

  return (
    <div className="mx-auto max-w-3xl px-6 py-16 lg:px-8 lg:py-20">
      <SectionTitle eyebrow="Apply" title="Apply to MASAR Talent Network"
        desc="Share your profile, portfolio, and role preference. We review candidates for remote creative opportunities with Saudi companies." />

      <div className="mt-10 rounded-[28px] border bg-white p-6 shadow-[0_12px_40px_rgba(15,23,42,0.06)] lg:p-8" style={{ borderColor: brand.line }}>
        <form onSubmit={handleSubmit} className="grid gap-4">

          <input name="fullName" type="text" placeholder="Full Name" value={form.fullName} onChange={handleChange} required
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />
          ...
          <input name="yearsExperience" type="text" placeholder="Years of Experience" value={form.yearsExperience} onChange={handleChange}
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />

          <input name="portfolioUrl" type="url" placeholder="Portfolio URL" value={form.portfolioUrl} onChange={handleChange}
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />

          <input name="linkedinUrl" type="url" placeholder="LinkedIn URL" value={form.linkedinUrl} onChange={handleChange}
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />

          <input name="salaryExpectation" type="text" placeholder="Salary Expectation" value={form.salaryExpectation} onChange={handleChange}
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />

          <textarea name="notes" placeholder="Tell us about yourself" value={form.notes} onChange={handleChange} rows={5}
            className="rounded-2xl border px-4 py-3 text-sm outline-none" style={inputStyle} />
        ...
        {status && (
          <div className="mt-4 rounded-2xl px-4 py-3 text-sm"
            style={{ background: status.type === "success" ? "#ECFDF5" : "#FEF2F2", color: status.type === "success" ? "#065F46" : "#991B1B" }}>
            {status.message}
          </div>
        )}
      </div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <div className="grid gap-8 lg:grid-cols-[1fr_0.95fr]">
        <div className="rounded-[34px] border bg-white p-10 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-14" style={{ borderColor: brand.line }}>
          <SectionTitle eyebrow="Contact" title="Ready to build your team in Egypt through a more structured model?"
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to discuss building a team in Egypt.")}`}
              target="_blank" rel="noreferrer"
              ...
              style={{ background: brand.primary }}>
              Contact on WhatsApp <MessageCircle className="h-4 w-4" />
            </a>
          </div>
        </div>
        <div className="rounded-[34px] border bg-white p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-10" style={{ borderColor: brand.line }}>
          ...
          <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to discuss building a team in Egypt.")}`}
            ...
            style={{ background: brand.secondary, color: brand.primary }}>
            Start on WhatsApp <MessageCircle className="h-4 w-4" />
          </a>
        </div>
      </div>
    </div>
...
function AppLayout() {
  return (
    <div style={{ background: brand.bg }} className="min-h-screen text-slate-900">
      ...
      <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to know more about your services.")}`}
        target="_blank" rel="noreferrer"
