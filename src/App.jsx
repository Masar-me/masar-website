...
const faqs = [
  { q: "What exactly does Masar do?", a: "Masar helps Saudi and GCC companies hire and operate employees in Egypt through a structured local operating model. This includes onboarding, contracts, payroll workflows, HR administration, and ongoing employee support." },
  { q: "Is Masar a recruitment agency?", a: "Not in the traditional sense. Masar is positioned as a workforce infrastructure partner. Recruitment can be part of the journey, but the core value is in the operational structure around the employee after hiring." },
  { q: "Who is this service best suited for?", a: "Masar is ideal for Saudi and GCC companies that want to build an Egypt-based team without creating the full administrative and employment structure on their own from day one." },
  { q: "How is pricing usually structured?", a: "Masar can be priced as a setup fee, recruitment fee, monthly administration fee, and optional add-ons depending on scope and workforce structure." },
  { q: "Can Masar support scaling later?", a: "Yes. The model is designed to start lean, then support expansion into a broader, more stable team structure as the client grows." },
];
...
function Logo() {
  ...
  if (!broken) {
    return <img src={company.logoPath} alt="Masar logo" className="h-11 w-auto rounded-xl object-contain" onError={() => setBroken(true)} />;
  }
  return <div className="flex h-11 w-11 items-center justify-center rounded-2xl text-sm font-semibold text-white" style={{ background: brand.primary }}>M</div>;
}

function SectionTitle({ eyebrow, title, desc }) {
  return (
    <div className="max-w-3xl">
      <div className="text-sm font-medium uppercase tracking-[0.22em]" style={{ color: brand.secondary }}>{eyebrow}</div>
      <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl" style={{ color: brand.primary }}>{title}</h2>
      {desc ? <p className="mt-4 text-base leading-8" style={{ color: brand.muted }}>{desc}</p> : null}
    </div>
  );
}
  ...
  return (
    <div className="border-b px-6 py-3 text-sm" style={{ background: brand.primary, borderColor: "rgba(255,255,255,0.08)", color: "white" }}>
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 lg:px-2">
        ...
        <a href={`https://wa.me/${company.whatsapp}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition hover:opacity-90" style={{ background: brand.secondary, color: brand.primary }}>
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {navItems.map((item) => {
            ...
            return (
              <NavLink key={item.path} to={item.path} className="rounded-xl px-4 py-2 text-sm transition" style={{ background: active ? brand.primary : "transparent", color: active ? "white" : brand.muted }}>
                {item.label}
              </NavLink>
            );
          })}
        ...
        <div className="flex items-center gap-3">
          <Link to="/contact" className="hidden rounded-2xl px-5 py-3 text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5 md:inline-flex" style={{ background: brand.primary }}>
            Book a consultation
          </Link>
          <button className="flex h-11 w-11 items-center justify-center rounded-2xl border bg-white md:hidden" style={{ borderColor: brand.line }} onClick={() => setMenuOpen((s) => !s)}>
          ...
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="border-t bg-white px-6 py-4 md:hidden" style={{ borderColor: brand.line }}>
            <div className="grid gap-2">
              {navItems.map((item) => {
                const active = location.pathname === item.path;
                return (
                  <NavLink key={item.path} to={item.path} onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 text-left text-sm transition" style={{ background: active ? brand.primary : "transparent", color: active ? "white" : brand.muted }}>
                    {item.label}
                  </NavLink>
                );
              })}
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 lg:grid-cols-[1.1fr_0.9fr_0.9fr] lg:px-8">
        ...
        <div>
          ...
          <div className="mt-4 grid gap-2">
            {navItems.map((item) => <NavLink key={item.path} to={item.path} className="w-fit text-sm transition" style={{ color: brand.muted }}>{item.label}</NavLink>)}
          </div>
        </div>
        <div>
    ...
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-3">
        {pillars.map((item, index) => {
          ...
          return (
            <motion.div key={item.title} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.07 }} className="rounded-[30px] border bg-white p-8 shadow-[0_16px_45px_rgba(15,23,42,0.06)]" style={{ borderColor: brand.line }}>
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl text-white" style={{ background: brand.primary }}><Icon className="h-5 w-5" /></div>
              <h3 className="mt-6 text-xl font-semibold tracking-tight" style={{ color: brand.primary }}>{item.title}</h3>
    ...
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        {services.map((service, index) => {
          ...
          return (
            <motion.div key={service.title} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.06 }} className="rounded-[30px] border bg-white p-8 shadow-[0_16px_45px_rgba(15,23,42,0.06)]" style={{ borderColor: brand.line }}>
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl text-white" style={{ background: brand.primary }}><Icon className="h-5 w-5" /></div>
...
function PackagesPage() {
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <SectionTitle eyebrow="Packages" title="Flexible commercial models based on team size and operational depth." desc="Masar can be priced as a setup fee, recruitment fee, monthly administration fee, and optional add-ons depending on scope and workforce structure." />
      <div className="mt-12 grid gap-6 lg:grid-cols-3">
        {packages.map((pkg, index) => (
          <motion.div key={pkg.name} initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.45, delay: index * 0.07 }} className="rounded-[32px] border p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)]" style={{ borderColor: pkg.featured ? brand.primary : brand.line, background: pkg.featured ? brand.primary : brand.card, color: pkg.featured ? "white" : brand.primary }}>
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="rounded-full px-3 py-1 text-xs font-medium uppercase tracking-[0.18em]" style={{ background: pkg.featured ? "rgba(255,255,255,0.08)" : "#F5F2E8", color: pkg.featured ? "white" : brand.secondary }}>{pkg.tag}</div>
            </div>
        ...
        {faqs.map((item, index) => (
          <div key={item.q} className="rounded-[24px] border bg-white px-6 py-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]" style={{ borderColor: brand.line }}>
            <button onClick={() => setOpenIndex(openIndex === index ? -1 : index)} className="flex w-full items-center justify-between gap-4 text-left">
              <div className="text-base font-semibold tracking-tight" style={{ color: brand.primary }}>{item.q}</div>
              <ChevronDown className={`h-5 w-5 shrink-0 transition ${openIndex === index ? "rotate-180" : "rotate-0"}`} style={{ color: brand.secondary }} />
            </button>
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20">
      <div className="grid gap-8 lg:grid-cols-[1fr_0.95fr]">
        <div className="rounded-[34px] border bg-white p-10 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-14" style={{ borderColor: brand.line }}>
          <SectionTitle eyebrow="Contact" title="Ready to build your team in Egypt through a more structured model?" desc="Let’s discuss the roles you need, your preferred team structure, and the right commercial model for your company." />
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-[24px] p-5" style={{ background: "#FCFBF8" }}><div className="flex items-center gap-3 text-sm font-semibold" style={{ color: brand.primary }}><Mail className="h-4 w-4" /> Email</div><div className="mt-2 text-sm" style={{ color: brand.muted }}>{company.email}</div></div>
            <div className="rounded-[24px] p-5" style={{ background: "#FCFBF8" }}><div className="flex items-center gap-3 text-sm font-semibold" style={{ color: brand.primary }}><Phone className="h-4 w-4" /> Phone</div><div className="mt-2 text-sm" style={{ color: brand.muted }}>{company.phone}</div></div>
            <div className="rounded-[24px] p-5 sm:col-span-2" style={{ background: "#FCFBF8" }}><div className="flex items-center gap-3 text-sm font-semibold" style={{ color: brand.primary }}><MapPin className="h-4 w-4" /> Coverage</div><div className="mt-2 text-sm" style={{ color: brand.muted }}>{company.location}</div></div>
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
            <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to discuss building a team in Egypt.")}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium text-white transition hover:-translate-y-0.5" style={{ background: brand.primary }}>
              Contact on WhatsApp
        ...
        <div className="rounded-[34px] border bg-white p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)] lg:p-10" style={{ borderColor: brand.line }}>
          <div className="text-sm font-medium uppercase tracking-[0.22em]" style={{ color: brand.secondary }}>Next step</div>
          <h3 className="mt-4 text-2xl font-semibold tracking-tight" style={{ color: brand.primary }}>Share your hiring needs directly</h3>
          <p className="mt-3 text-sm leading-7" style={{ color: brand.muted }}>Tell Masar the team size, target roles, and preferred start timeline. This keeps the first discussion fast and practical.</p>
          <div className="mt-8 space-y-4">
            {["Company name", "Target roles needed", "Expected team size", "Preferred start timeline", "Operational notes"].map((item) => (
              <div key={item} className="rounded-2xl border px-4 py-3 text-sm" style={{ borderColor: brand.line, color: brand.muted, background: "#FCFBF8" }}>{item}</div>
            ))}
          </div>
          <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to discuss building a team in Egypt.")}`} target="_blank" rel="noreferrer" className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium transition hover:-translate-y-0.5" style={{ background: brand.secondary, color: brand.primary }}>
            Start on WhatsApp
            <MessageCircle className="h-4 w-4" />
          </a>
        </div>
...
function AppLayout() {
  return (
    <div style={{ background: brand.bg }} className="min-h-screen text-slate-900">
      ...
      <a href={`https://wa.me/${company.whatsapp}?text=${encodeURIComponent("Hello Masar, I would like to know more about your services.")}`} target="_blank" rel="noreferrer" className="fixed bottom-6 right-6 z-40 inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium shadow-lg transition hover:-translate-y-0.5" style={{ background: brand.secondary, color: brand.primary }}>
        <MessageCircle className="h-4 w-4" /> WhatsApp
      </a>
      <Footer />
      <SpeedInsights />
